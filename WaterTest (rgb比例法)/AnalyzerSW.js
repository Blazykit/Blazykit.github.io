...
const phColorTable = [
    ...
    { ph: 4,  color: [183, 92, 87] },
    { ph: 5,  color: [197, 105, 91] },
    { ph: 6,  color: [196, 201, 126] },
    { ph: 7,  color: [140, 164, 85] },
    { ph: 8,  color: [100, 150, 88] },
    { ph: 9,  color: [73, 76, 82] },
    { ph: 10, color: [106, 32, 52] },
];
...
const turbidityStd = [
    { boxId: "redBox2", turbidity: 0 },    // 0 JTU
    { boxId: "redBox3", turbidity: 40 },   // 40 JTU
...
function getContrast(box) {
    ...
    for (let i = 0; i < imageData.length; i += 4) {
        // RGB to grayscale luminance (ITU-R BT.709)
    ...
    // 最暗10%、最亮10%
    let percent = 0.1;
    let nPart = Math.max(1, Math.floor(n * percent));

    let black = LArr.slice(0, nPart).reduce((s, v) => s + v, 0) / nPart;
    let white = LArr.slice(n - nPart, n).reduce((s, v) => s + v, 0) / nPart;

    // 避免0除
    if (black + white === 0) return 0;
    let contrast = (white - black) / (white + black);
    return contrast;
}

function getTurbidityCurveTable() {
    return turbidityStd.map(std => {
        let box = document.getElementById(std.boxId);
        if (!box) return null;
        let contrast = getContrast(box);
        return { contrast: contrast, turbidity: std.turbidity };
    });
}

function getTurbidityFromCurve(mainContrast, stdCurveTable) {
    // 確保資料按 JTU 對應正確排序（如果已確定無誤可省略）
    stdCurveTable = stdCurveTable.sort((a, b) => a.turbidity - b.turbidity);
    let x = stdCurveTable.map(e => e.contrast);    // 對比度
    let y = stdCurveTable.map(e => e.turbidity);   // JTU：0, 40, 100

    // 直接做二次曲線回歸
    let fit = quadraticFit(x, y);
    let turbidity = fit.a * mainContrast * mainContrast + fit.b * mainContrast + fit.c;
    return turbidity;
}
...
function getOxygenLabTable() {
    return oxygenStd.map(std => {
        ...
        return {
            lab: color.b / Math.max(color.r, 1),   // 用 B/R 當 lab
...
// Delta-E 線性插值法
function getOxygenByLab(mainColor, stdLabTable, tolerance=0.005) {
     let mainBr = mainColor.b / Math.max(mainColor.r, 1);

    for (let i = 0; i < stdLabTable.length; i++) {
        let diff = Math.abs(mainBr - stdLabTable[i].lab);
        if (diff < tolerance) {
    ...
    for (let i = 0; i < stdLabTable.length; i++) {
        let diff = Math.abs(mainBr - stdLabTable[i].lab);
        let weight = 1 / Math.max(diff, 1e-6);
        result += stdLabTable[i].concentration * weight;
        totalWeight += weight;
    }
...
function getInterpolatedPhValue(r, g, b) {
    ...
    return { ph: Math.round(ph * 10) / 10, t }; 
}
...
function getPhByNormalizedRGB(r, g, b) {
    const target = normalizeRGB(r, g, b);

    let totalWeight = 0;
    let weightedPh = 0;

    for (const entry of phColorTable) {
        const refRGB = normalizeRGB(...entry.color);
        const dist = rgbDistance(target, refRGB);
        const weight = 1 / Math.max(dist, 1e-6); // 越近越重

        weightedPh += entry.ph * weight;
        totalWeight += weight;
    }

    return Math.round((weightedPh / totalWeight) * 10) / 10;
}
...
function getPhFromRgbFormula(r, g, b) {
    const ratio = g / Math.max(b, 1); // 避免除以 0
    return Math.max(4, Math.min(10, 2.01 * ratio + 2.47)); // 限制在合理範圍內
}


function getPhByBRatio(r, g, b) {
    const sum = r + g + b;
    if (sum === 0) return 7;
    const bRatio = b / sum;
    const ph = 279.975 * bRatio - 398.129 * bRatio * bRatio - 39.419;
    return Math.max(4, Math.min(10, Math.round(ph * 10) / 10));
}

    // pH值
    if (labelText === "酸鹼值" && mainColor) {
    const { ph: ph1, t } = getInterpolatedPhValue(mainColor.r, mainColor.g, mainColor.b);
    const ph2 = getPhByNormalizedRGB(mainColor.r, mainColor.g, mainColor.b);    // Normalized RGB 加權
    const ph3 = getPhFromRgbFormula(mainColor.r, mainColor.g, mainColor.b);     // 公式法

    resultHtml += `<div style="margin-top:8px;">
        <b>主圈對應酸鹼值：</b><br>
        <span style="font-size:18px;color:#1976D2;">比例t:${t.toFixed(2)} </span><br>
        <span style="font-size:18px;color:#1976D2;">${ph1.toFixed(1)} (Lab插值)</span><br>
        <span style="font-size:18px;color:#43a047;">${ph2.toFixed(1)} (RGB比對)</span><br>
        <span style="font-size:18px;color:#EF6C00;">${ph3.toFixed(1)} (G/B公式)</span>
    ...
    if (stdCurveTable.some(e => e === null)) {
        ...
        resultHtml += `<div style="margin-top:8px;">
            ...
            <br>主圈對比度: ${mainContrast.toFixed(4)}
        </div>`;
    }
}
    // 未選擇
